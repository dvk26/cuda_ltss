...
// --------- forward_internal: d_x_ -> d_c5_ + MSE (d_target_) ----------
...
// --------- backward_internal: backprop + SGD update ----------
...
// --------- public API: forward / train_batch ----------

void GPUAutoencoder::forward(const Tensor& x_host, Tensor& y_host)
{
    ...
    CUDA_CHECK(cudaMemcpy(d_x_, x_host.raw().data(), sz, cudaMemcpyHostToDevice));

    // target không cần set (chỉ reconstruct)
    float loss_dummy;
    // forward_internal đang tính MSE, nhưng mình có thể bỏ phần MSE:
    // cách nhanh: copy d_target_ = d_x_ rồi gọi forward_internal
    CUDA_CHECK(cudaMemcpy(d_target_, d_x_, sz, cudaMemcpyDeviceToDevice));
    loss_dummy = forward_internal();

    // copy output về host
    y_host.resize(N_, 3, H_, W_);
    CUDA_CHECK(cudaMemcpy(y_host.raw().data(), d_c5_, sz, cudaMemcpyDeviceToHost));
}

float GPUAutoencoder::train_batch(const Tensor& x_host, float lr)
{
    if (x_host.N() != N_ || x_host.C() != 3 || x_host.H() != H_ || x_host.W() != W_) {
        throw std::runtime_error("GPUAutoencoder::train_batch: input shape mismatch");
    }

    size_t sz = nchw_size(N_, 3, H_, W_) * sizeof(float);

    // H2D: copy input + target (autoencoder target = input)
    CUDA_CHECK(cudaMemcpy(d_x_, x_host.raw().data(), sz, cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_target_, x_host.raw().data(), sz, cudaMemcpyHostToDevice));

    // forward + MSE + d_dy_
    float loss = forward_internal();

    // backward + update
    backward_internal(lr);
