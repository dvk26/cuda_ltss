...
private:
    ...
    bool keep_partial_;            // true: chỉ giữ 4% của tập dữ liệu gốc; false: giữ toàn bộ
...
public:
    ...
    void load(const std::string& dir, bool keep_partial=true) {
        ...
        if (keep_partial_) { // giữ 4% dữ liệu gốc để chạy quy mô nhỏ(2000 ảnh train, 400 ảnh test)
    ...
    // by CIFAR convention (or actual size if keep_partial_ is false):
